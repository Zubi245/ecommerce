...
export const Header = () => {
  ...
  return (
    <>
      ...
      >
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center h-20">
            ...
            <Link href="/" className="flex items-center space-x-2 group">
              <div className="relative">
                ...
                <h1 className="relative text-2xl md:text-3xl font-serif font-bold tracking-tight">
                  <span className="text-brand-black group-hover:text-gray-800 transition-colors duration-300">
                    SAIM
                  </span>
              ...
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`relative px-4 py-2 text-sm font-medium uppercase tracking-wider transition-all duration-300 group ${
                    isActive(link.href) ? 'text-gold-600' : 'text-gray-700 hover:text-gold-600'
                  }`}
                >
                  <span className="relative z-10">{link.label}</span>
                  <span
                    className={`absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-gold-500 to-gold-600 transform origin-left transition-transform duration-300 ${
            ...
            {/* RIGHT: Cart Icon */}
            <Link href="/cart" className="relative group flex-shrink-0">
      ...
      {/* Mobile Menu Overlay & Sidebar (Mobile Only) */}
      {isMenuOpen && (
        <div
          className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] md: flex"
