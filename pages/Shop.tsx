  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        // Fetch products from API, fallback to localStorage
        let fetchedProducts: Product[] = [];
        let fetchedCategories: string[] = [];
        
        try {
          fetchedProducts = await productsApi.getAll();
          fetchedCategories = await categoriesApi.getAll();
        } catch {
          // Fallback to localStorage
          fetchedProducts = db.getEnabledProducts();
          fetchedCategories = [...new Set(fetchedProducts.map(p => p.category).filter(Boolean))];
        }
        ...
        if (fetchedProducts.length > 0) {
          const prices = fetchedProducts.map(p => p.salePrice || p.price).filter(p => typeof p === 'number' && !isNaN(p));
          if (prices.length > 0) {
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      {filteredProducts.length === 0 ? (
        ...
        <>
          <p className="text-sm text-gray-500 mb-6 animate-fade-in">
